# Variables for the auth database module (Cloud SQL for PostgreSQL).
# Separate Cloud SQL instance for the OIDC/auth service — isolated from the
# product database to enforce security boundary and independent scaling.
...
variable "db_name" {
  type        = string
  description = "Name of the default database to create within the auth Cloud SQL instance."
  default     = "iexcel_auth"
}
